...
import legacyFormatMenuData from '../../helpers/legacyFormatMenuData';
...
class LegacyPlantMenu extends Component {
  state = {
    qty: 0,
    status: 'pending',
    search: '',
      ...
      <View
        ...
        <View style={{display: 'flex', flexDirection: 'row'}}>
          ...
          <TouchableOpacity
            ...
            <Ionicons name="search" size={fonts.h2} color={colors.purpleB} />
          </TouchableOpacity>
        </View>
      </View>
    );
  ...
  renderTabs() {
    ...
    const rows = legacyFormatMenuData(
    ...
    const planted =
      this.props.order.type === types.INITIAL_PLANTING
        ? getPlantedList(this.props.drafts)
        : getPlantedList(this.props.beds);
    const plantData =
      this.props.order.type === types.INITIAL_PLANTING
        ? this.props.drafts
        : this.props.beds;
    ...
    return (
      <View
        style={{display: 'flex', flexDirection: 'row', marginTop: units.unit3}}>
        <TouchableOpacity
          style={{flex: 1}}
          onPress={() => this.setStatus('pending')}>
          <View
            style={{
      ...
      <View style={{backgroundColor: colors.greenE10}}>
        <View
          ...
          {rows.map((column, index) => {
            // check drafts for plant
            const plantIsSaved = planted.find(
              plant => plant.key === column.key,
            );
                  ...
                  <View
                    ...
                    {!column.isEmpty && (
                      <View
                        ...
                        {this.state.selectedPlants.find(
                          (p, i) => p.key === index + 1,
                        ) && (
                          <View
                            style={{
  ...
  renderListMenu() {
    const listItems = legacyFormatMenuData(
      this.props.vegetables,
      this.props.herbs,
      this.props.fruit,
      this.state.search,
              ...
              <TouchableOpacity
                ...
                <Card
                  ...
                  <View
                    ...
                    <View>
                      <Text>
                        {li.id.name} {li.id.common_type.name}
                      </Text>
                      <Text
                        style={{
                          ...fonts.label,
                          color: planted ? colors.greenE25 : colors.greenB,
                        }}>
                        {planted ? 'Planted' : 'Pending'}
                      </Text>
                    </View>
                  </View>
...
LegacyPlantMenu = connect(mapStateToProps, null)(LegacyPlantMenu);

export default LegacyPlantMenu;

module.exports = LegacyPlantMenu;
