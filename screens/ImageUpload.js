...
import {
  createConversation,
  getConversations,
} from '../actions/conversations/index';
...
class ImageUpload extends Component {
  ...
  async componentDidMount() {
    if (this.props.route.params.order.type === types.INITIAL_PLANTING) {
      ...
      this.props.route.params.order.type === types.FULL_PLAN ||
      this.props.route.params.order.type === types.ASSISTED_PLAN
    ) {
      ...
      const plantsToBeReplaced = this.props.plantActivities.filter(
        plantActivity =>
          plantActivity.type === types.DECEASED ||
          (plantActivity.type === types.HARVESTED &&
            plantActivity.harvest === 'full'),
      );
      let plantReplacements = [];

      // group plant replacements by common type and plant type
      plantsToBeReplaced.forEach(plantToBeReplaced => {
        const matchIndex = plantReplacements.findIndex(
          replacement =>
            replacement.plant.common_type._id ===
              plantToBeReplaced.plant.common_type._id &&
            replacement.plant._id === plantToBeReplaced.plant._id,
        );
        if (matchIndex > 0) {
  ...
  async selectImages() {
    ...
    launchImageLibrary(
      ...
      res => {
        ...
        if (!res.didCancel) {
          const imgWithNoDimensions = res.assets?.find(
          ...
          if (imgWithNoDimensions) {
            ...
            return alert(
              `Your file "${imgWithNoDimensions.fileName}" has no dimensions, please select another image and try again.`,
            );
          } else {
            // upload images
            this.finish(res);
    ...
    Promise.all(updateBeds).then(async () => {
      ...
      Promise.all(createPlantActivities).then(async () => {
        ...
        if (
          ...
          !order.customer.payment_info.plan_id
        ) {
          ...
          if (
            maintenancePlan !== types.FULL_PLAN &&
            maintenancePlan !== types.ASSISTED_PLAN
          ) {
            // get maintance plan
            const plan = this.props.plans.find(
              plan => plan._id === maintenancePlan,
            );
          ...
          const orderDescription =
            maintenancePlan === types.FULL_PLAN
              ? vars.orderDescriptions.customer.fullPlan
              : vars.orderDescriptions.customer.assistedPlan;
          ...
          const subscription = await this.props.createSubscription(
            newSubscription,
          );
  ...
  async processCropRotation() {
    ...
    Promise.all(updateBeds).then(async () => {
      ...
      Promise.all(createPlantActivities).then(async () => {
        ...
        const maintanceOrder = this.props.orders.list.find(
          o =>
            o.customer._id === order.customer._id &&
            (o.type === types.FULL_PLAN || o.type === types.ASSISTED_PLAN),
        );
    ...
    Promise.all(createAnswers).then(async () => {
      const orderDate =
        order.type === types.FULL_PLAN
          ? moment(order.date).add(1, 'week').startOf('day')
          : moment(order.date).add(2, 'weeks').startOf('day');
      const orderDescription =
        order.type === types.FULL_PLAN
          ? vars.orderDescriptions.customer.fullPlan
          : vars.orderDescriptions.customer.assistedPlan;
      ...
      const questionIndex = this.props.questions.findIndex(
        question => question.placement === 6,
      );
      const question = this.props.questions[questionIndex];
      const answerIndex = this.props.answers.findIndex(
      ...
      const conversations = await this.props.getConversations(
        `users=${order.customer._id},${this.props.user._id}`,
        true,
      );
  ...
  render() {
    const {isLoading, selectedImages} = this.state;
    const order = this.props.route.params.order;
