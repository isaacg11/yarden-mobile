...
class Substitution extends Component {
  ...
  async setPlantList() {
    ...
    const plants = await this.props.getPlants(
      `common_type=${this.props.route.params.selectedPlant.id.common_type._id}&quadrant_size=${this.props.route.params.selectedPlant.id.quadrant_size}`,
      true,
    );
    ...
    plants.forEach(plant => {
      if (
        `${plant.name} ${plant.common_type.name}` !==
        `${this.props.route.params.selectedPlant.id.name} ${this.props.route.params.selectedPlant.id.common_type.name}`
      ) {
  ...
  async confirm() {
    ...
    alert('Your plant has been successfully substituted.', 'Success!', () => {
  ...
  async getBedWithSubstitution(substitute) {
    ...
    beds.forEach(bed => {
      bed.plot_points.forEach(row => {
        row.forEach(column => {
          if (column.plant) {
            const matchingCommonType =
              column.plant.id.common_type._id ===
              selectedPlant.id.common_type._id;
            const matchingPlantName =
              column.plant.id.name === selectedPlant.id.name;
  ...
  async substitutePlant(substitute) {
    ...
    if (this.props.route.params.order.type === types.INITIAL_PLANTING) {
      ...
      if (substitute.id.category.name === types.VEGETABLE) {
        ...
        // update plant list
        await this.props.updatePlantList(this.props.plantList._id, {
          vegetables: updatedVegetables,
        });

        // get plant list
        await this.props.getPlantList(
      ...
      // update bed
      await this.props.updateBed(bedWithSubstitution._id, {
        plot_points: bedWithSubstitution.plot_points,
      });
      ...
      // get draft to update
      const draftToUpdate = this.props.drafts.find(
        draft => draft.key === bedWithSubstitution.key,
      );

      // update draft
      await this.props.updateDraft(draftToUpdate._id, {
        plot_points: bedWithSubstitution.plot_points,
      });
      ...
      // update quote
      await this.props.updateQuote(this.props.route.params.order.bid._id, {
        line_items: lineItems,
      });
    } else if (this.props.route.params.order.type === types.CROP_ROTATION) {
      // if crop rotation {...}
      let vegetables = this.props.plantList.vegetables;
      let herbs = this.props.plantList.herbs;
      let fruit = this.props.plantList.fruit;
  ...
  async updateQty(plants, substitute) {
    const existingMatchingPlantIndex = plants.findIndex(
      plant => plant.id._id === substitute.id._id,
    );
    ...
    const plantToReplaceIndex = plants.findIndex(
      plant => plant.id._id === this.props.route.params.selectedPlant.id._id,
    );
  ...
  async createVarietal() {
    ...
    if (!this.state.varietalName) {
      // show warning
      alert('Please enter a varietal name', 'Uh Oh!');
    } else {
      // show loading indicator
      this.setState({isLoading: true});
      ...
      const plantsWithMatchingName = await this.props.getPlants(
        `name=${plantName}&common_type=${similarVarietal.common_type._id}`,
        true,
      );
      if (plantsWithMatchingName.length > 0) {
        ...
        const newVarietal = {
          ...
          image:
            'https://yarden-garden.s3.us-west-1.amazonaws.com/plant-images/placeholder.png',
          name: plantName,
        ...
        const newBedsRequest = {
          ...
          body:
            ...
            '<p style="margin-bottom: 15px;">A new plant varietal has been created by <u>' +
            this.props.user.email +
            '</u>, please review and update as needed.</p>' +
            '<p><b>New Plant</b></p>' +
            '<p>' +
            '"' +
            `${varietal.name} ${similarVarietal.common_type.name}` +
            '"' +
            '</p>',
        };
        ...
        alert('The new varietal has been added to the plant list', 'Success!');
      }
    }
  }

  render() {
    const {plants, substitutePlant, isLoading, varietalName, isEditingName} =
      this.state;
            ...
            <View style={{width: '45%'}}>
              ...
              <View
                ...
                <Text
                  ...
                  {substitutePlant
                    ? `${substitutePlant.name} ${substitutePlant.common_type.name}`
                    : 'Plant Name'}
                </Text>
                <Text
                  style={{
                  ...
                  {substitutePlant
                    ? `${substitutePlant.botanical_type.name}`
                    : 'Botanical Type'}
                </Text>
              </View>
            </View>
          ...
          {isEditingName && (
            <View>
              <View
                ...
                <Text>
                  Enter the varietal name. For example, if you are adding "Sweet
                  Corn", only enter "Sweet".
                </Text>
              </View>
              <Input
              ...
              <View
                ...
                <TouchableOpacity onPress={() => this.createVarietal()}>
                  <Ionicons
                    name={'checkmark'}
