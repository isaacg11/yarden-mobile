...
class Checkout extends Component {
    ...
    async componentDidMount() {
        ...
        if (this.props.route.params.plantSelections) {
            ...
            const combinedPlants = combinePlants(this.props.route.params.plantSelections);
    ...
    getPlantsList(quote) {
        // get plant selection
        const plantSelection = this.props.route.params.plantSelections.find((selection) => selection.quoteTitle === quote.title);
    ...
    async scheduleNewOrder(approval, quote, isGarden) {
        ...
        if (this.props.route.params.vendor) newOrder.vendor = this.props.route.params.vendor._id;
    ...
    async approve() {
        ...
        if (charge.status !== 200) {
            ...
            if (this.props.route.params.isChangeOrder) { // if approval is for a change order {...}

                // update change order as "approved"
                await this.props.updateChangeOrder(this.props.route.params._id, { status: 'approved', approval: approval._id });

                // assign order value
                order = this.props.route.params.order;
            } if (this.props.route.params.isPurchase) { // if approval is for a purchase {...}
                ...
                order = await this.scheduleNewOrder(approval.data, this.props.route.params, isGarden);
            }
    ...
    async updateGardenInfo(vegetables, fruit, herbs) {
        ...
        if (this.props.user.garden_info) {
            ...
            if (this.props.route.params.plan) gardenInfo.maintenance_plan = this.props.route.params.plan;
        }
