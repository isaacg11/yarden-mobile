...
class Product extends Component {
  ...
  async componentDidMount() {
    ...
    // get product rules
    const productRules = await this.props.getRules(
    ...
    if (productRules.length > 0) {
      ...
      const productMeasurements = getProductMeasurements(
    ...
    if (selectedProduct.name === 'garden bed') {
      ...
      productMaterials.forEach(material => {
        const isIrrigationItem = this.props.irrigation.find(
          irr => irr.item._id === material._id,
        );
