  ...
  async componentDidMount() {
    ...
    if (order.type === types.FULL_PLAN || order.type === types.ASSISTED_PLAN) {
      ...
      if (this.props.reports.length > 0) {
        ...
        // get report type
        const initialPlantingReportType = await this.props.getReportType(
          `name=${types.INITIAL_PLANTING}`,
        );

        // get previous reports for customer
        await this.props.getReports(
          `customer=${order.customer._id}&type=${initialPlantingReportType._id}`,
        );
      ...
      // if order is for crop rotation {...}

      // get current maintenance plan type
      const maintenanceType = order.customer.garden_info.maintenance_plan;

      // get report type
      const maintenanceReportType = await this.props.getReportType(
        `name=${maintenanceType}`,
      );

      // get previous reports for customer
      await this.props.getReports(
        `customer=${order.customer._id}&type=${maintenanceReportType._id}`,
      );
    ...
    return (
      <View>
        <Header type="h6" style={{color: colors.purpleB}}>
          {this.state.daysInterval}{' '}
          {this.state.daysInterval > 1 ? 'Days' : 'Day'}
        </Header>
      </View>
    );
  }
    ...
    return (
      <View>
        <Header type="h6" style={{color: colors.purpleB}}>
          ...
          {this.state.minuteDuration > 1 ? 'Minutes' : 'Minute'}
        </Header>
      </View>
    );
  }
          ...
          <View style={{padding: units.unit3 + units.unit4}}>
            <View
              ...
              <View style={{paddingHorizontal: units.unit6}}>
                <Label style={{textAlign: 'center', marginTop: units.unit4}}>
                  Previous Schedule
                </Label>
                <Text style={{textAlign: 'center'}}>
                  {wateringSchedule.length > 0
                    ? formatWateringSchedule(
                        wateringSchedule[wateringSchedule.length - 1].result,
                      )
                    : 'No schedule found'}
                </Text>
              </View>
              ...
              <View style={{marginTop: units.unit5}}>
                {!customIntervals && (
                  <View>
                    ...
                    <Slider
                      ...
                      value={daysInterval}
                      onValueChange={value =>
                        this.setState({daysInterval: Math.round(value)})
                      }
                    />
                  </View>
                )}
                <View
                  ...
                  <Link
                    text={
                      customIntervals ? 'Daily Intervals' : 'Custom Intervals'
                    }
                    onPress={() => {
                ...
                {customIntervals && (
                  <View>
                    <View
                      ...
                      />
                      <Paragraph style={{marginLeft: units.unit4}}>
                        Monday
                      </Paragraph>
                    </View>
                    <View
              ...
              <View style={{marginTop: units.unit5}}>
                ...
                <Slider
                  ...
                  onValueChange={value =>
                    this.setState({minuteDuration: Math.round(value)})
                  }
                />
              </View>
